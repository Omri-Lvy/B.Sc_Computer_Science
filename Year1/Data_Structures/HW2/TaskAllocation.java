 ...
 * At any given time, a task at the front of either line may be removed from the
 * data structure.
 * 
 * 
 * 
...
public class TaskAllocation {
	...
	/**
	 * Creates a task allocation queue from an a array of tasks, ordered according
	 * to the creation time
	 * 
	 * @param arr a given array of TaskElements. The heapIndex field of the elements
	 *            in arr might be incorrect
	 */
	...
	public Task allocatePriorityTask() {
		...
		TaskElement task = heap.extractMax();
		q.remove(task);
		return task.t;
	}
	...
	public Task allocateRegularTask() {
		TaskElement task = q.dequeue();
		heap.remove(task.heapIndex);
		return task.t;
	}
