...
public class Sorting {
	 ...
	 */
	 private static double QuickSelect(double[] arr, int r,int p, int i) {
		 if (p == r)
				return arr[r];
			int q = partition(arr, p, r);
			int m = q - r + 1;
			if (i == m)
				return arr[q];
			if (i < m)
				return QuickSelect(arr, r, q-1, i);
			else
				return QuickSelect(arr, q+1, p, i - m);
	    }
...
//	 */
	 ...
	 * 
	 * @param arr - the array to be sorted
	 * @param p   - the first index in the range
