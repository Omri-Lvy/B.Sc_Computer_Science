...
public class JackTokenizer {
    ...
    private List<Character> SYMBOL_LIST = Arrays.asList('{', '}', '(', ')', '[', ']', '.', ',', ';', '+', '-', '*',
            '/', '&', '|', ',', '<', '>', '=', '~');
    private String stringConstant = "";
    private boolean stringConstantBuilder = false;

    private List<Character> ops = Arrays.asList('+', '-', '=', '<', '>');
    ...
    private void splitSymbols (String word) {
        ...
        if (!word.equals(tkn) && !word.equals("") && !stringConstantBuilder) {
            tokens.add(word);
        }
    ...
    public boolean isOp () {
        return ops.contains(token.charAt(0));
    }

    public String getSymbolString(char symbol) {
        switch (symbol) {
            case '<': return "&lt;";
            case '>': return "&gt;";
            case '&': return "&amp;";
